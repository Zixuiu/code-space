...
from tkinter import messagebox  # 🚀 必须导入，否则提交报错
...
class EnterpriseSalesApp:
    ...
    def show_add_lead_dialog(self):
        ...
        dialog.title("录入新客户")
        dialog.geometry("480x480") # 稍微加高一点确保舒适
        ...
        # ================= 1. 顶部装饰与标题 (从上往下布局) =================
        tk.Frame(dialog, bg=self.accent_blue, height=6).pack(fill=tk.X)
        
        title_frame = tk.Frame(dialog, bg="white")
        title_frame.pack(fill=tk.X, padx=30, pady=(25, 15))
        tk.Label(title_frame, text="新增销售线索", font=("微软雅黑", 18, "bold"), bg="white", fg="#0F172A").pack(side=tk.LEFT)
        tk.Label(title_frame, text="✏️", font=("Arial", 18), bg="white").pack(side=tk.RIGHT)

        # ================= 2. 底部按钮区 (必须第二个布局，固定在底部) =================
        btn_frame = tk.Frame(dialog, bg="white")
        btn_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=30, pady=25) # 使用 side=tk.BOTTOM 强制沉底
        
        def submit():
            comp = entries["客户名称"].get().strip()
            if not comp:
                messagebox.showwarning("提示", "客户名称不能为空", parent=dialog)
                return
        ...
        # 取消按钮
        cancel_btn = tk.Button(btn_frame, text="取消", font=("微软雅黑", 11), bg="#F1F5F9", fg="#64748B", 
                            bd=0, padx=30, pady=10, cursor="hand2", command=dialog.destroy,
                            activebackground="#E2E8F0", activeforeground="#334155")
        cancel_btn.pack(side=tk.RIGHT, padx=(15, 0))
        
        # 提交按钮
        submit_btn = tk.Button(btn_frame, text="✔ 确认提交", font=("微软雅黑", 11, "bold"), 
                            bg=self.accent_blue, fg="white", bd=0, padx=30, pady=10, 
        ...
        form_frame.pack(fill=tk.BOTH, expand=True, padx=30) # expand=True 此时只会占据按钮上方的剩余空间
        ...
        for field in fields:
            ...
            tk.Label(frame, text=field, font=("微软雅黑", 11), bg="white", fg="#64748B").pack(anchor="w", pady=(0, 5))
            ...
            if field == "当前阶段":
                ...
                entry = tk.Entry(frame, font=("微软雅黑", 12), bg="#F8FAFC", relief="flat", 
                                highlightthickness=1, highlightcolor=self.accent_blue, 
                                highlightbackground="#E2E8F0", insertbackground=self.accent_blue)
    ...
    def setup_layout(self):
            ...
            # 顶部标题栏 (模拟官方顶栏)
            self.header = tk.Frame(self.main_area, bg="white", height=70)  # 👉 在这里才创建了 self.header
