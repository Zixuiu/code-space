...
from PyQt5.QtWidgets import (
    ...
    QRadioButton, QFileDialog, QStackedWidget, QGraphicsOpacityEffect
)
...
class _HoverCloseButton(QWidget):
    """macOS Photos 风格的圆形关闭按钮 - 自绘两条交叉线

    设计要点：
    - 完全自绘：半透明黑色填充圆 + 白色交叉直线（不是字符 ×）
    - hover 时背景加深、线条加粗变纯白
    - pressed 时变 iOS 红色（轻微动效）
    - 始终可见，不依赖 hover 才出现
    """
    def __init__(self, parent_widget, on_click, size=22):
        ...
        # 必须使用 WA_TranslucentBackground 让 paintEvent 的透明区域能正确合成
        self.setAttribute(Qt.WA_TranslucentBackground, True)
        # 不要 WA_NoSystemBackground，它会导致某些环境下 paintEvent 不被调用
        # 不使用 QGraphicsOpacityEffect，它会与自绘透明产生冲突
    ...
    def paintEvent(self, event):
        ...
        # 1. 背景圆：iOS 系统红（按下） / 半透明深黑（hover） / 半透明黑（普通）
        if self._pressed:
            bg = QColor(255, 59, 48, 235)        # iOS 红
            line_color = QColor(255, 255, 255, 250)
            line_w = max(1.4, s * 0.09)
        elif self._hovered:
            bg = QColor(20, 20, 20, 220)
            line_color = QColor(255, 255, 255, 255)
            line_w = max(1.3, s * 0.08)
        else:
            bg = QColor(0, 0, 0, 180)            # 提高不透明度，确保可见
            line_color = QColor(255, 255, 255, 240)
            line_w = max(1.5, s * 0.09)           # 加粗线条

        painter.setPen(Qt.NoPen)
        painter.setBrush(QBrush(bg))
        painter.drawEllipse(rect)

        # 2. 1px 细描边（macOS Photos 风格）
        if not self._pressed:
            border = QColor(255, 255, 255, 50 if self._hovered else 30)
        ...
        painter.setPen(pen)
        # 第一条: 左上 → 右下
        painter.drawLine(QPointF(cx - line_len, cy - line_len),
                         QPointF(cx + line_len, cy + line_len))
        # 第二条: 右上 → 左下
        painter.drawLine(QPointF(cx + line_len, cy - line_len),
                         QPointF(cx - line_len, cy + line_len))


def _create_hover_close_button(parent_widget, on_click, size=22):
    """
...
class FolderManager(QDialog):
    ...
    def view_images(self, folder_path):
        ...
        for i, img_file in enumerate(image_files):
            ...
            delete_btn = _create_hover_close_button(
                image_container,
                on_click=lambda checked=False, file_name=img_file: delete_image(file_name),
                size=24
            )
            delete_btn.move(container_size - 26, 2)  # 右上角位置（留 2px 边距）
            ...
            if i < len(self.image_actions):
                ...
                if action_text.startswith("按键:"):
                    ...
                    action_label.setStyleSheet(f"background-color: #007AFF; color: #1E1E2E; padding: 2px 6px; border-radius: 4px; font-weight: bold; font-family: 'PingFang SC', 'Microsoft YaHei UI', 'Helvetica Neue', 'Segoe UI', sans-serif; font-size: {action_font_size}px; cursor: pointer;")
                ...
                elif action_text.startswith("滚动:"):
                    # 滚动操作显示为可点击的标签
                    scroll_display = action_text.replace("滚动: ", "")
                    action_label = QLabel(f"🔄 {scroll_display}")
                    action_label.setMinimumSize(60, control_height)
                    action_label.setStyleSheet(f"background-color: #89B4FA; color: #1E1E2E; padding: 2px 6px; border-radius: 4px; font-weight: bold; font-family: 'PingFang SC', 'Microsoft YaHei UI', 'Helvetica Neue', 'Segoe UI', sans-serif; font-size: {action_font_size}px; cursor: pointer;")
                    action_label.setAlignment(Qt.AlignCenter)
                    action_label.setToolTip(f"滚动: {scroll_display}")
                    action_label.mousePressEvent = lambda event, img_idx=i, fp=folder_path: self.show_scroll_input_dialog(img_idx, fp)
                    controls_layout.addWidget(action_label)
                    ...
                    action_label.setStyleSheet(f"background-color: #A6E3A1; color: #1E1E2E; padding: 2px 4px; border-radius: 4px; font-weight: bold; font-family: 'PingFang SC', 'Microsoft YaHei UI', 'Helvetica Neue', 'Segoe UI', sans-serif; font-size: {action_font_size}px;")
                    action_label.setAlignment(Qt.AlignCenter)
                    controls_layout.addWidget(action_label)
                    ...
                    action_label.setFixedSize(100, control_height)
                    action_label.setStyleSheet(f"background-color: #FAB387; color: #1E1E2E; padding: 2px 4px; border-radius: 4px; font-weight: bold; font-family: 'PingFang SC', 'Microsoft YaHei UI', 'Helvetica Neue', 'Segoe UI', sans-serif; font-size: {action_font_size}px;")
                    action_label.setAlignment(Qt.AlignCenter)
                    controls_layout.addWidget(action_label)
...
class AutoRecorderApp(QMainWindow):
    ...
    def _update_replay_ui(self):
        ...
        if hasattr(self, 'floating_replay_btn'):
            ...
            _open_style = """
                ...
                QPushButton:hover {
                    background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                        stop:0 #6BDC82, stop:1 #30D158);
                }
                QPushButton:pressed {
                    background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                        stop:0 #2BA846, stop:1 #28A745);
                    padding-top: 2px;
                }
            """
            if self.replay_enabled:
    ...
    def create_record_tab(self):
            ...
            def paintEvent(self, event):
                ...
                if self._recording:
                    ...
                    if self._pressed:
                        ...
                        gradient.setColorAt(0.55, QColor("#FF3B30"))
                ...
                text_color = QColor("#1a1a2e") if not self._recording else QColor("#FF3B30")
        ...
        float_btn = PillButton("悬浮窗口",
                               color_top="#5AC8FA", color_mid="#007AFF", color_bottom="#0062CC",
                               text_color="white")
    ...
    def create_help_tab(self):
        ...
        help_content = QLabel("""
            <div style="font-size: 18px; line-height: 2; color: #2C3E50; font-family: 'PingFang SC', 'Microsoft YaHei UI', 'Helvetica Neue', 'Segoe UI', sans-serif;">
            <p><b style="color: #FF453A;">⌨️ 快捷键</b></p>
            <p>&nbsp;&nbsp;· 键：开始/停止录制</p>
            <p>&nbsp;&nbsp;Home 键：回到主窗口</p>
            <br/>
            <p><b style="color: #FF453A;">🎬 录制流程</b></p>
            <p>&nbsp;&nbsp;点击"录制"按钮开始录制操作</p>
            <p>&nbsp;&nbsp;再次点击或按 · 键停止录制</p>
            <br/>
            <p><b style="color: #FF453A;">📁 流程管理</b></p>
            <p>&nbsp;&nbsp;在"流程管理"标签页管理录制</p>
            <br/>
                ...
                def create_replay_callback(fp):
                    def callback():
                        ...
                        if not self.replay_enabled:
                            # 修复:不再静默 return,而是自动开启回放 + 明确提示
                            # 这样用户按快捷键就能直接回放,不需要先点"回放已关闭"按钮
                            self.replay_enabled = True
                            # 同步 UI 按钮状态(主界面 + 悬浮窗)
                            if hasattr(self, 'replay_btn') and self.replay_btn is not None:
                                try:
                                    self.replay_btn.setText("● 回放已开启")
                                except Exception:
                                    pass
                            if hasattr(self, 'floating_replay_btn') and self.floating_replay_btn is not None:
                                try:
                                    self.floating_replay_btn.setText("● 回放已开启")
                                except Exception:
                                    pass
                            self.log_signal.emit(f"⚡ 快捷键触发,自动开启回放模式: {os.path.basename(fp)}")
                            self.debug_print(f"快捷键触发,自动开启回放模式")
    ...
    def replay_folder_operations(self, folder_path, stop_check=None):
        """回放指定文件夹中的操作，返回是否成功完成
        
        Args:
            folder_path: 要回放的流程文件夹路径
            stop_check: 可选的停止检查函数（用于多组合技并行时各runner独立停止）
        """
        # 组合技调试日志
        print(f"[COMBO DEBUG] replay_folder_operations called: folder_path={folder_path}")
        print(f"[COMBO DEBUG] login_manager.current_user={self.login_manager.current_user if hasattr(self, 'login_manager') else 'NO LOGIN_MANAGER'}")
        print(f"[COMBO DEBUG] current_user={getattr(self, 'current_user', 'NO CURRENT_USER')}")
        
        # 检查用户是否已登录（支持后备方案）
        current_user = self.login_manager.current_user if hasattr(self, 'login_manager') else None
        if not current_user:
            current_user = getattr(self, 'current_user', None)
        if not current_user:
            print(f"[COMBO DEBUG] 登录检查失败: 所有用户字段均为空")
        ...
        try:
            ...
            if has_image:
                ...
                success_count, total = replay_coordinate_operations(recording_data, original_path, replay_interval=self.replay_interval, consider_color=True, match_timeout=self.replay_timeout, stop_check=stop_check, skip_cache_clear=stop_check is not None)
            else:
                # 坐标录制 - 只有坐标数据
                self.debug_print("检测到坐标录制，使用坐标回放")
                success_count, total = replay_coordinates_only(recording_data, replay_interval=self.replay_interval, stop_check=stop_check)

            # 流程执行完成（不管每个步骤是否成功，只要流程执行了就算完成）
            # 区分"自然完成"和"被停止"两种情况，避免 stop 后还报"回放完成"误导
            if is_replay_stopped():
                self.debug_print(f"回放已停止: 已完成 {success_count}/{total} 个步骤")
            else:
            ...
            # 只在非组合技runner调用时（即直接回放）重置is_replaying状态
            # 组合技runner各自管理自己的生命周期
            if stop_check is None:
                self.is_replaying = False
                self.update_replay_playback_indicator()
...
def start_app():
    ...
    # 设置全局默认字体为微软雅黑
    default_font = QFont()
    default_font.setFamily("PingFang SC")
    # 设置字体策略以获得更好的渲染效果
...
class ComboSkillManager(QDialog):
    ...
    def initUI(self):
        ...
        # 简洁标题
        title = QLabel("🎯 组合技列表")
        title.setStyleSheet(f"font-size: 16px; font-weight: bold; color: {ACCENT}; font-family: 'PingFang SC', 'Microsoft YaHei UI', 'Helvetica Neue', 'Segoe UI', sans-serif;")
        title.setAlignment(Qt.AlignCenter)
        layout.addWidget(title)

        # 简化说明
        tip = QLabel("单击运行 | 右键编辑/删除")
        tip.setStyleSheet(f"color: {MUTED}; font-size: 11px; font-family: 'PingFang SC', 'Microsoft YaHei UI', 'Helvetica Neue', 'Segoe UI', sans-serif;")
    ...
    def run_combo_skill(self):
        ...
        # 检查这个特定的组合技是否已经在运行
        if hasattr(self.parent, 'runners') and skill_id in self.parent.runners:
            if self.parent.runners[skill_id].isRunning():
                QMessageBox.warning(self, "提示", f"组合技 '{skill['name']}' 正在运行中，请先停止后再执行")
                return
        
        # 清除这个runner的旧引用（如果存在但已停止）
        if hasattr(self.parent, 'runners') and skill_id in self.parent.runners:
            old_runner = self.parent.runners[skill_id]
            if not old_runner.isRunning():
                del self.parent.runners[skill_id]
        ...
        # 创建执行线程前，清理旧缓存释放内存
        from image_recognition import clear_image_cache as _clear_cache
        _clear_cache()
        
        # 清除全局停止标记，确保不会继承上一次停止的状态
        from image_recognition import clear_replay_stop_flag as _clear_stop_flag
        _clear_stop_flag()
        
        # 创建执行线程
        runner = ComboSkillRunner(skill, self.parent)
        runner.skill_id = skill_id  # 给runner添加唯一标识
        self.parent.runners[skill_id] = runner
        
        # 连接信号
        runner.finished.connect(lambda success, msg, sid=skill_id: self.on_run_finished_parallel(success, msg, sid))
        runner.log_signal.connect(lambda msg, sid=skill_id: self.on_log_message_parallel(msg, sid))
        runner.step_signal.connect(lambda step_info, sid=skill_id: self.on_step_changed_parallel(step_info, sid))
        runner.start()
        
        # 刷新表格显示运行状态
        if hasattr(self.parent, 'combo_tab') and hasattr(self.parent.combo_tab, 'combo_table'):
            self.parent.load_combo_skills_to_table(self.parent.combo_tab.combo_table)

    def on_run_finished_parallel(self, success, message, skill_id):
        """执行完成回调（并行版本）"""
        if hasattr(self.parent, 'runners') and skill_id in self.parent.runners:
            del self.parent.runners[skill_id]
        ...
        if self.parent and hasattr(self.parent, 'append_log'):
            if success:
                self.parent.append_log(f"[{skill_name}] 完成: {message}")
            else:
                self.parent.append_log(f"[{skill_name}] 错误: {message}")
        
        remaining_running = any(r.isRunning() for r in self.parent.runners.values())
        if not remaining_running:
            from image_recognition import clear_image_cache as _clear_cache
            _clear_cache()
    ...
    def on_log_message_parallel(self, message, skill_id):
        """接收日志消息并发送到主窗口日志窗口（并行版本）"""
        # 获取skill名称
        skill_name = ""
        for skill in self.combo_skills:
            if skill.get('name', '') == skill_id:
        ...
        if self.parent and hasattr(self.parent, 'append_log'):
            self.parent.append_log(f"[{skill_name}] {message}")
    
    def on_step_changed_parallel(self, step_info, skill_id):
        """步骤变化回调（并行版本）"""
        if self.parent and hasattr(self.parent, '_on_combo_step_changed'):
            self.parent._on_combo_step_changed(step_info, skill_id)
    ...
    def _get_combo_config_path(self):
        """获取组合技配置文件的共享路径"""
        try:
            from utils import get_recordings_path
            recordings_dir = get_recordings_path()
            shared_dir = os.path.dirname(recordings_dir)
        except:
            shared_dir = os.path.dirname(os.path.abspath(__file__))
        return os.path.join(shared_dir, "combo_skills.json")

    def load_combo_skills(self):
        """加载组合技配置"""
        try:
            config_path = self._get_combo_config_path()
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
...
class ComboSkillEditDialog(QDialog):
    ...
    def __init__(self, parent=None, skill_data=None):
        ...
        # 深拷贝流程数据，避免对话框内的修改污染原始数据
        # 否则拖拽排序、删除流程等操作即使点"取消"也会影响原始数据
        self.flows = copy.deepcopy(self.skill_data.get('flows', []))
        # 确保至少有一行
        if len(self.flows) == 0:
            self.flows.append({
    ...
    def initUI(self):
        ...
        # 监测模式开关
        self.monitor_check = QCheckBox("🔍 监测模式")
        self.monitor_check.setChecked(self.skill_data.get('monitor_mode', False))
        self.monitor_check.setToolTip("启用后，此组合技在后台持续监测屏幕，发现目标图片时暂停主流程并执行对应操作")
        ...
        """)
        top_layout.addWidget(self.monitor_check)
    ...
    def create_flow_item_widgets(self, tree_item, index, flow_data, is_else=False):
        ...
        if not is_else:
            ...
            else_btn.setStyleSheet("background: #FF453A; color: white; padding: 4px 8px; font-size: 10px;")
        ...
        wait_time_spin = QDoubleSpinBox()
        wait_time_spin.setRange(0.1, 999999)  # 0.1-999999秒，支持小数
        wait_time_spin.setDecimals(1)  # 保留1位小数
        wait_time_spin.setSingleStep(0.5)  # 步长0.5秒
        wait_time_spin.setValue(flow_data.get('wait_timeout', 30.0))  # 默认30秒
            ...
            for i in range(len(self.flows)):
                if i != current_flow_index:  # 不显示跳转到自己的选项
                    ...
                    if flow_action and not flow_action.startswith('跳转_') and flow_action != 'end':
                        # 优先使用磁盘上实际的文件夹名(若磁盘上存在 action 的"更新版本",用最新的)
                        display_name = flow_action
                        if disk_folders and flow_action not in disk_folders:
                            for folder in disk_folders:
                                if flow_action in folder:
                                    display_name = folder
                                    break
                    else:
    ...
    def on_action_type_changed(self, index, type_combo, detail_combo, is_else):
        ...
        if action_type == 'end':
            ...
            if detail_combo.count() > 0:
                detail_combo.setCurrentIndex(0)
                selected_value = detail_combo.itemData(0)
                if is_else:
                    if self.flows[index].get('else_branch'):
    ...
    def on_action_detail_changed(self, index, type_combo, detail_combo, is_else):
        ...
        if selected_value:
            ...
            # 当某个流程的 action 名称变了,刷新所有其他流程的"跳转"下拉框
            # (跳转下拉框显示的是目标流程的 action,需要同步更新)
            self._refresh_all_goto_dropdowns()

    def _refresh_all_goto_dropdowns(self):
        """刷新所有流程的'跳转流程'下拉框,使其显示最新的流程名"""
        for widget_data in self.flow_widgets:
            action_type_combo = widget_data.get('action_type_combo')
            action_detail_combo = widget_data.get('action_detail_combo')
            flow_index = widget_data.get('flow_index', 0)
            if not action_type_combo or not action_detail_combo:
                continue
            # 只在'跳转流程'类型时刷新
            if action_type_combo.currentData() != 'goto':
                continue
            # 记录当前选中的跳转目标(保存 data 值)
            current = action_detail_combo.currentData()
            # 重新加载
            self.load_goto_options(action_detail_combo, flow_index)
            # 恢复选中（临时阻塞信号）
            if current:
                old_state = action_detail_combo.blockSignals(True)
                try:
                    for j in range(action_detail_combo.count()):
                        if action_detail_combo.itemData(j) == current:
                            action_detail_combo.setCurrentIndex(j)
                            break
                finally:
                    action_detail_combo.blockSignals(old_state)
    
    def load_goto_options(self, combo, current_index):
        """加载跳转流程选项 - 会从磁盘读取最新的文件夹名,避免 action 字段过时"""
        from utils import get_recordings_path
        recordings_dir = get_recordings_path()
            ...
            for i in range(len(self.flows)):
                if i != current_index:  # 不显示自己
                    ...
                    if flow_action and not flow_action.startswith('跳转_') and flow_action != 'end':
                        ...
                        if disk_folders and flow_action not in disk_folders:
                            for folder in disk_folders:
                                # 子串匹配:说明 folder 是在 action 基础上加了前缀/后缀(被改名了)
                                if flow_action and flow_action in folder:
                                    display_name = folder
                                    break
                        display_name = f"流程{i+1} ({display_name})"
                    else:
                        display_name = f"流程{i+1}"
                    combo.addItem(f"跳转到{display_name}", f"跳转_{i}")
        finally:
    ...
    def delete_flow(self):
        ...
        deleted_index = len(self.flows) - 1
        
        # 删除最后一个流程
        self.flows.pop()
        
        # 更新所有跳转索引：删除指向被删流程的跳转，调整大于被删索引的跳转
        for flow in self.flows:
            # 处理主流程的跳转
            action = flow.get('action', '')
            if action.startswith('跳转_'):
                target = int(action.split('_')[1])
                if target == deleted_index:
                    # 跳转到被删除的流程，清除跳转
                    flow['action'] = '无'
                elif target > deleted_index:
                    # 跳转到被删流程之后，索引减1
                    flow['action'] = f'跳转_{target - 1}'
            ...
            if else_action.startswith('跳转_'):
                ...
                if target == deleted_index:
                    # 跳转到被删除的流程，清除跳转
                    else_branch['action'] = '无'
                elif target > deleted_index:
                    # 跳转到被删流程之后，索引减1
                    else_branch['action'] = f'跳转_{target - 1}'
    ...
    def get_skill_data(self):
        ...
        return {
            ...
            "note": note,
            "monitor_mode": self.monitor_check.isChecked()
        }
...
class ComboSkillRunner(QThread):
    ...
    def __init__(self, skill_data, parent=None):
        ...
        self._exec_thread = None  # 用于Python线程模式
        self._pause_event = threading.Event()  # 暂停事件（set=暂停, clear=运行）
        self._pause_event.clear()
        self._stop_event = threading.Event()  # 停止事件（set=请求停止，用于中断 sleep）
        self._stop_event.clear()
        self.monitor_mode = skill_data.get('monitor_mode', False)  # 是否为监测模式
        self.monitor_target_runner = None  # 监测目标runner（暂停/恢复这个runner）
        self._monitor_check_interval = 1.0  # 监测截图间隔（秒）从0.3改为1.0，大幅降低CPU

    def _interruptible_sleep(self, duration):
        """可中断的 sleep：等待 duration 秒，但如果收到停止信号则立即返回。

        返回 True 表示被中断（停止），False 表示正常等待完成。
        """
        if duration <= 0:
    ...
    def reset(self):
        """重置runner状态，确保下次运行从第一个流程重新开始

        重要：不要在这里把 self.running 设回 True。
        - 当前线程正在被请求停止（stop_combo_skill 已先设 self.running=False），
          如果这里再设回 True，线程会继续执行流程，破坏"重置"语义。
        - 下次启动时，run_selected_combo_skills 会创建新的 ComboSkillRunner 实例，
          新实例的 __init__ 会把 self.running 初始化为 True。
        """
        try:
            # 触发停止事件，让所有可中断 sleep 立即返回
            self._stop_event.set()
            self.current_step_info = {}
            # 清空暂停事件，确保不会卡在暂停状态
            self._pause_event.clear()
        except Exception as e:
            print(f"[ComboSkillRunner.reset] 重置状态失败: {e}")

    def isRunning(self):
        """重写isRunning，支持Python threading.Thread模式"""
    ...
    def pause(self):
        """暂停此runner的执行"""
        self.log_signal.emit("⏸ 收到暂停信号")
        self._pause_event.set()

    def resume(self):
        """恢复此runner的执行"""
        self._pause_event.clear()
        self.log_signal.emit("▶ 收到恢复信号")
    ...
    def run(self):
        ...
        try:
            ...
            self.log_signal.emit(f"\n{'#'*60}")
            self.log_signal.emit(f"# ▶ 开始执行组合技: {skill_name}")
            self.log_signal.emit(f"#   流程数: {total_flows}, 循环次数: {loop_count}")
            ...
            executed_step_count = 0  # 实际执行的流程计数
            skipped_step_count = 0  # 跳过的流程计数
            ...
            while self.running and current_loop < loop_count and total_step_count < max_iterations:
                ...
                _loop_start_ts = _time.time()

                self.log_signal.emit(f"\n{'='*60}")
                self.log_signal.emit(f"# 🔄 [轮次 {current_loop}/{loop_count}] 开始 (累计已执行={executed_step_count}, 累计跳过={skipped_step_count})")
                self.log_signal.emit(f"{'='*60}")
                ...
                while self.running and current_flow_index < len(flows) and total_step_count < max_iterations:
                    total_step_count += 1
                    _step_start_ts = _time.time()
                    _flow_no = current_flow_index + 1

                    # 检查停止信号（在进入每个流程前都检查）
                    if not self.running:
                        self.log_signal.emit(f"⏹ [轮{current_loop}] 流程 {_flow_no}/{total_flows} 进入前检测到停止信号，跳出内循环")
                        break
                    ...
                    self.log_signal.emit(
                        f"📍 [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 条件='{condition_text}', 操作='{action_text}'"
                    )
                    ...
                    # 检查条件
                    self.log_signal.emit(f"🔎 [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 正在检查条件 '{condition_text}'...")
                    should_execute, should_stop = self.should_execute_flow(flow)
                    self.log_signal.emit(
                        f"{'✅' if should_execute else '❌'} [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 条件结果 should_execute={should_execute}, should_stop={should_stop}"
                    )

                    if should_stop:
                        # 需要停止组合技
                        self.log_signal.emit(f"⏹ [轮{current_loop}] 流程 {_flow_no}/{total_flows}: should_stop=True, 停止组合技")
                        self.running = False
                        break

                    if should_execute:
                        # 条件满足，执行主流程
                        self.log_signal.emit(f"✅ [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 条件满足，执行主分支操作")

                        if not action:
                            # 没有设置执行操作
                            self.log_signal.emit(f"⚠️ [轮{current_loop}] 流程 {_flow_no}/{total_flows} 未设置执行操作，跳过")
                            skipped_step_count += 1
                            current_flow_index += 1

                        elif action == 'end':
                            # 结束组合技
                            self.log_signal.emit("🏁 [主分支] 执行操作: 结束组合技")
                            self.running = False
                            break
                            ...
                            try:
                                ...
                                if 0 <= target_index < len(flows):
                                    self.log_signal.emit(f"➡️ [主分支] 跳转到流程 {target_index + 1} (从流程 {_flow_no} 跳转)")
                                    current_flow_index = target_index
                                    continue
                                else:
                                    self.log_signal.emit(f"❌ [主分支] 跳转目标 {target_index + 1} 超出范围 [1, {len(flows)}], 改为顺序执行")
                                    current_flow_index += 1
                            except Exception as _je:
                                self.log_signal.emit(f"❌ [主分支] 跳转目标无效: {action} ({_je})")
                                current_flow_index += 1

                        else:
                            # 执行流程（action是流程名称）
                            self.log_signal.emit(f"🚀 [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 准备执行子流程 '{action}'...")
                            import sys
                            sys.stdout.flush()
                            _flow_ok = self.execute_flow(flow)
                            _flow_cost = _time.time() - _step_start_ts
                            if not _flow_ok:
                                # 执行失败：不再终止整个组合技，仅跳过本流程继续
                                self.log_signal.emit(
                                    f"⚠️ [轮{current_loop}] 流程 {_flow_no}/{total_flows} (子流程 '{action}') 执行失败 ❌ 耗时 {_flow_cost:.2f}s,跳过本流程,继续后续流程"
                                )
                                skipped_step_count += 1
                                current_flow_index += 1
                                continue
                            sys.stdout.flush()
                            self.log_signal.emit(
                                f"✅ [轮{current_loop}] 流程 {_flow_no}/{total_flows} (子流程 '{action}') 执行完成 ⏱ 耗时 {_flow_cost:.2f}s"
                            )
                            ...
                            if delay_after > 0 and self.running:
                                self.log_signal.emit(f"⏳ [轮{current_loop}] 流程 {_flow_no}/{total_flows} 后等待 {delay_after} 秒...")
                                if self._interruptible_sleep(delay_after):
                                    self.log_signal.emit(f"⏹ [轮{current_loop}] 等待被中断")
                    else:
                        # 条件不满足，检查是否有else分支
                        ...
                        if else_branch:
                            ...
                            self.log_signal.emit(
                                f"⚡ [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 主条件不满足,准备执行 Else 分支 (条件='{else_condition_text}', 操作='{else_action_text}')"
                            )
                            ...
                            self.log_signal.emit(f"⚡ [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 执行 Else 分支")

                            if not else_action:
                                self.log_signal.emit(f"⚠️ [轮{current_loop}] 流程 {_flow_no}/{total_flows} Else分支未设置执行操作，跳过")
                                skipped_step_count += 1
                                current_flow_index += 1
                            elif else_action == 'end':
                                self.log_signal.emit("🏁 [Else分支] 执行操作: 结束组合技 (Else分支)")
                                self.running = False
                                break
                                ...
                                try:
                                    ...
                                    if 0 <= target_index < len(flows):
                                        self.log_signal.emit(f"➡️ [Else分支] 跳转到流程 {target_index + 1} (从流程 {_flow_no} 跳转)")
                                        current_flow_index = target_index
                                        continue
                                    else:
                                        self.log_signal.emit(f"❌ [Else分支] 跳转目标 {target_index + 1} 超出范围 [1, {len(flows)}], 改为顺序执行")
                                        current_flow_index += 1
                                except Exception as _je:
                                    self.log_signal.emit(f"❌ [Else分支] 跳转目标无效: {else_action} ({_je})")
                                    current_flow_index += 1
                            else:
                                self.log_signal.emit(f"🚀 [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 准备执行 Else 子流程 '{else_action}'...")
                                import sys
                                sys.stdout.flush()
                                _flow_ok = self.execute_flow(else_branch)
                                _flow_cost = _time.time() - _step_start_ts
                                if not _flow_ok:
                                    # 执行失败：仅跳过本流程，继续后续
                                    self.log_signal.emit(
                                        f"⚠️ [轮{current_loop}] 流程 {_flow_no}/{total_flows} (Else子流程 '{else_action}') 执行失败 ❌ 耗时 {_flow_cost:.2f}s,跳过"
                                    )
                                    skipped_step_count += 1
                                    current_flow_index += 1
                                else:
                                    sys.stdout.flush()
                                    self.log_signal.emit(
                                        f"✅ [轮{current_loop}] 流程 {_flow_no}/{total_flows} (Else子流程 '{else_action}') 执行完成 ⏱ 耗时 {_flow_cost:.2f}s"
                                    )
                                    executed_step_count += 1
                                    current_flow_index += 1
                                    ...
                                    if delay_after > 0 and self.running:
                                        self.log_signal.emit(f"⏳ [轮{current_loop}] 流程 {_flow_no}/{total_flows} Else分支后等待 {delay_after} 秒...")
                                        if self._interruptible_sleep(delay_after):
                                            self.log_signal.emit(f"⏹ [轮{current_loop}] Else等待被中断")
                        else:
                            # 没有else分支，跳到下一个流程
                            self.log_signal.emit(
                                f"⏭️ [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 条件不满足且无 Else 分支,跳过"
                            )
                            skipped_step_count += 1
                            current_flow_index += 1

                    # 流程间添加短暂延迟
                    self._interruptible_sleep(0.1)  # 缩短延迟到0.1秒

                    # 检查是否需要暂停（被监测组合技暂停）
                    if self._pause_event.is_set():
                        self.log_signal.emit(f"⏸ [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 监测到目标，暂停本组合技...")
                        while self.running and self._pause_event.is_set():
                            self._interruptible_sleep(0.1)
                        if not self.running:
                            self.log_signal.emit(f"⏹ [轮{current_loop}] 暂停期间被停止")
                            break
                        self.log_signal.emit(f"▶ [轮{current_loop}] 流程 {_flow_no}/{total_flows}: 监测处理完成，恢复执行")

                # 一轮循环结束
                _loop_cost = _time.time() - _loop_start_ts
                if current_flow_index >= len(flows):
                    self.log_signal.emit(f"\n{'='*60}")
                    self.log_signal.emit(f"# ✅ [轮次 {current_loop}/{loop_count}] 完成 ⏱ 耗时 {_loop_cost:.2f}s (累计已执行 {executed_step_count} 步, 累计跳过 {skipped_step_count} 步)")
                    self.log_signal.emit(f"{'='*60}")
                else:
                    self.log_signal.emit(f"\n{'='*60}")
                    self.log_signal.emit(f"# ⚠️ [轮次 {current_loop}/{loop_count}] 未完成所有流程 (current_flow_index={current_flow_index}/{len(flows)}) ⏱ 耗时 {_loop_cost:.2f}s")
                    self.log_signal.emit(f"{'='*60}")

                if not self.running:
                    break

            _combo_total_cost = _time.time() - _combo_start_ts
            self.log_signal.emit(f"\n{'#'*60}")
            if not self.running:
                self.log_signal.emit(f"# ⏹ 组合技已停止 (共执行 {current_loop} 轮, 总耗时 {_combo_total_cost:.2f}s, 共执行 {executed_step_count} 步, 跳过 {skipped_step_count} 步)")
            else:
                self.log_signal.emit(f"# 🏁 组合技执行完成 (共 {current_loop} 轮, 总耗时 {_combo_total_cost:.2f}s, 共执行 {executed_step_count} 步, 跳过 {skipped_step_count} 步)")
            self.log_signal.emit(f"{'#'*60}")

            if not self.running:
                self.finished.emit(False, f"组合技已停止，共执行 {current_loop} 轮")
            else:
            ...
            _combo_total_cost = _time.time() - _combo_start_ts
            self.log_signal.emit(f"\n{'!'*60}")
            self.log_signal.emit(f"❌ 组合技执行异常 (已运行 {_combo_total_cost:.2f}s): {e}")
            import traceback
            self.log_signal.emit(f"异常堆栈:\n{traceback.format_exc()}")
            self.log_signal.emit(f"{'!'*60}")
    ...
    def run_monitor(self):
        """监测模式执行方法（优化版：低CPU/内存占用）
        
        在后台持续截图检测目标图片，发现后暂停主runner执行对应流程，完成后恢复
        """
        if self.monitor_target_runner is None:
            self.log_signal.emit("[监测] 错误：未指定监测目标组合技，请通过批量运行启动此监测组合技")
            self.finished.emit(False, "监测失败：未指定目标组合技")
            return
        ...
        try:
            ...
            # 预加载所有条件图片到内存（避免反复读磁盘）
            template_cache = {}
            for flow in flows:
                condition_image = flow.get('condition_image', '')
                if condition_image and os.path.exists(condition_image):
                    try:
                        from PIL import Image
                        import numpy as np
                        img = Image.open(condition_image)
                        if img.mode != 'RGB':
                            img = img.convert('RGB')
                        template_cache[condition_image] = np.array(img)
                    except Exception:
                        pass
            
            self.log_signal.emit(f"\n{'#'*60}")
            self.log_signal.emit(f"# 🔍 启动监测组合技（持续监测模式）: {skill_name}")
            self.log_signal.emit(f"#   监测 {len(flows)} 个条件，监测间隔 {self._monitor_check_interval}秒")
            self.log_signal.emit(f"#   ⚠ 持续运行中，请通过停止按钮终止")
            self.log_signal.emit(f"{'#'*60}")
            ...
            while self.running:
                ...
                for flow in flows:
                    ...
                    if condition == 'image_found' and condition_image:
                        template_arr = template_cache.get(condition_image)
                        if self.check_image_exists_fast(condition_image, template_arr):
                            should_trigger = True
                    
                    if should_trigger and self.running:
                        if self.monitor_target_runner:
                            self.log_signal.emit("⏸ [监测] 正在暂停主组合技...")
                            self.monitor_target_runner.pause()
                        
                        action = flow.get('action', '')
                        if action:
                            self.log_signal.emit(f"▶️ [监测] 执行操作: {action}")
                            self.execute_flow(flow)
                            self.log_signal.emit(f"✅ [监测] 操作执行完成: {action}")
                        
                        if self.monitor_target_runner:
                            self.monitor_target_runner.resume()
                            self.log_signal.emit("▶ [监测] 已恢复主组合技")
                        
                        # 执行后冷却：等待屏幕稳定再恢复监测
                        self.log_signal.emit("⏳ [监测] 冷却中... 等待屏幕稳定")
                        cooling = True
                        for _ in range(30):
                            if not self.running:
                                break
                ...
                # 每60秒（约60次检查）输出一次心跳日志
                if check_count % 60 == 0 and not cooling and self.running:
                    self.log_signal.emit(f"[监测] 持续运行中... 已监测 {check_count} 次")

                self._interruptible_sleep(self._monitor_check_interval)
            
        except Exception as e:
            self.log_signal.emit(f"[监测] 异常: {str(e)}")
            import traceback
            traceback.print_exc()
        finally:
            self.finished.emit(True, f"监测组合技已停止")
    ...
    def should_execute_flow(self, flow):
        ...
        if condition == 'always':
            ...
            # 等待图片出现，使用自定义等待时间
            timeout = flow.get('wait_timeout', 30)
            self.log_signal.emit(f"等待图片出现，最多等待 {timeout:.1f} 秒...")
    ...
    def wait_for_image(self, image_path, timeout=30):
        ...
        check_count = 0
        self.log_signal.emit(f"开始等待图片: {os.path.basename(image_path)}, 超时时间: {timeout:.1f}秒")
        ...
        if not self.running:
            self.log_signal.emit(f"已停止! 等待中断, 共检查 {check_count} 次, 耗时: {total_elapsed:.1f}秒")
        else:
    ...
    def check_image_exists(self, image_path):
        ...
        try:
            ...
            import numpy as np
            from PIL import Image
            from image_recognition import _mss_grab_array, _match_template_cuda, _get_shared_screenshot
            ...
            result = _match_template_cuda(screenshot_bgr, template_array)
            if result is None:
                result = cv2.matchTemplate(screenshot_bgr, template_array, cv2.TM_CCOEFF_NORMED)
            
            min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
            
            match_result = max_val >= 0.75
            status = "✅找到" if match_result else "❌未找到"
            self.log_signal.emit(f"[图片检测] {os.path.basename(image_path)}: 置信度={max_val:.3f} (阈值0.75) {status}")
            return match_result
    ...
    def check_image_exists_fast(self, image_path, template_array=None):
        """快速检查图片是否存在于屏幕上（使用预加载的模板，仅使用MSS）"""
        try:
            import cv2
            import numpy as np
            from image_recognition import _mss_grab_array, _match_template_cuda, _get_shared_screenshot
            ...
            if screenshot_bgr is None:
                self.log_signal.emit(f"[快速检测] ❌ MSS截图失败")
                return False
            ...
            match_result = max_val >= 0.75
            status = "✅找到" if match_result else "❌未找到"
            self.log_signal.emit(f"[快速检测] {os.path.basename(image_path)}: 置信度={max_val:.3f} (阈值0.75) {status}")
            return match_result
    ...
    def execute_flow(self, flow):
        ...
        try:
            ...
            if self.parent and hasattr(self.parent, 'replay_folder_operations'):
                # 传入stop_check，让回放函数能响应本runner的停止信号
                success = self.parent.replay_folder_operations(folder_path, stop_check=lambda: not self.running)
                if success:
