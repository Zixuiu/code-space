...
public class OrderController {
    ...
    */
    ...
    @GetMapping("/order/currentUser")
    @ApiOperation(value = "获取当前登录用户昵称", notes = "从会话/Cookie中解析登录用户名")
    @ResponseBody
    public ResponseVO<Map<String, Object>> currentUser(HttpServletRequest request) {
        Map<String, Object> data = new HashMap<>();
        String name = null;
        String[] keys = {"username", "name", "nickname", "user", "loginName", "nickName"};
        try {
            for (String k : keys) {
                Object v = request.getSession().getAttribute(k);
                if (v != null && !v.toString().trim().isEmpty()) { name = v.toString(); break; }
            }
        ...
        if (name == null) {
            ...
            if (cs != null) {
                for (Cookie c : cs) {
                    for (String k : keys) {
                        if (k.equalsIgnoreCase(c.getName()) && c.getValue() != null && !c.getValue().trim().isEmpty()) {
                            name = c.getValue().trim(); break;
                        }
                    }
                    if (name != null) break;
                }
        ...
        data.put("nickName", name == null ? "" : name);
