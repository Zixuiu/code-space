...
## 微信自动确认(新版本已支持不需要备注)
![微信自动确认](images/wechat_confirm.png)

## 支付宝当面付自动确认
![支付宝自动确认](images/alipay_confirm.png)
...
- 如果开启了无备注模式，那么递减金额的二维码要配置在原价的二维码文件夹下比如：5元，递减为0.01，4.99的二维码要在5.00的文件夹下新建一个4.99的文件夹，然后配置在这个目录下。
- 如果开启了无备注模式，同时下单的人数又超过了最大支持的同时下单人数，新下的订单会自动降级为备注模式，金额为原定金额+0.01，比如5元的订单会返回5.01
```
qr/
├── wechat/
│   ├── 5.00/
│   │   ├── 1.png/          # 5.00的二维码
│   │   ├── 5.01/          # 5.01的文件夹 当所有递减金额都被用完的时候启用这个
│   │   │   ├── 1.png/          # 5.01的二维码
│   │   ├── 4.99/          # 4.99的文件夹
│   │   │   ├── 1.png/          # 4.99的二维码
│   │   ├── 4.98/          # 4.98的文件夹
...
### 支付提示（1.0.9新版本已支持无需备注的方案。）
![支付提示](images/payment-help.png)

### 支付页面（1.0.9新版本已支持无需备注的方案。）
