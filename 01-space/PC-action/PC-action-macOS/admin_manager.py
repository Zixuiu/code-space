...
class AdminManager(QMainWindow):
    ...
    def on_load_error(self, error_message):
        ...
        retry_button.setFixedSize(80, 28)
    ...
    # ===== 商业化：价格配置编辑（作者后台改价，写入 pricing.json） =====
    def open_pricing_editor(self):
        try:
            from entitlement import get_pricing, save_pricing
            pricing = get_pricing()
            dlg = QDialog(self)
            dlg.setWindowTitle("价格配置（后台改价）")
            ...
            p1_price = QLineEdit(str(pricing.get('plan_1', {}).get('price', '')))
            p1_months = QLineEdit(str(pricing.get('plan_1', {}).get('months', '')))
            p1_desc = QLineEdit(pricing.get('plan_1', {}).get('desc', ''))
            p2_price = QLineEdit(str(pricing.get('plan_2', {}).get('price', '')))
            p2_months = QLineEdit(str(pricing.get('plan_2', {}).get('months', '')))
            p2_desc = QLineEdit(pricing.get('plan_2', {}).get('desc', ''))
            trial = QLineEdit(str(pricing.get('trial_days', 7)))
            ch_name = QLineEdit(pricing.get('channel_name', ''))
            ch_url = QLineEdit(pricing.get('channel_url', ''))

            form.addRow("基础会员 价格(元)", p1_price)
            form.addRow("基础会员 时长(月)", p1_months)
            form.addRow("基础会员 说明", p1_desc)
            form.addRow("高级会员 价格(元)", p2_price)
            form.addRow("高级会员 时长(月)", p2_months)
            form.addRow("高级会员 说明", p2_desc)
            form.addRow("试用天数", trial)
            form.addRow("渠道名称", ch_name)
            form.addRow("渠道链接", ch_url)
            dl.addLayout(form)
            ...
            def _do_save():
                try:
                    new_pricing = {
                        "plan_1": {"name": "基础会员",
                                   "price": float(p1_price.text() or 0),
                                   "months": int(p1_months.text() or 1),
                                   "desc": p1_desc.text()},
                        "plan_2": {"name": "高级会员",
                                   "price": float(p2_price.text() or 0),
                                   "months": int(p2_months.text() or 1),
                                   "desc": p2_desc.text()},
                        "trial_days": int(trial.text() or 7),
                        "channel_name": ch_name.text(),
                        "channel_url": ch_url.text(),
                    }
                    ok, msg = save_pricing(new_pricing)
                    if ok:
                        QMessageBox.information(dlg, "已保存", "价格配置已保存，前端会员中心将按新价格展示。")
                        dlg.accept()
                    else:
                        QMessageBox.warning(dlg, "保存失败", msg)
                except Exception as e:
                    QMessageBox.warning(dlg, "保存失败", f"请输入有效的数字: {e}")

            save_btn.clicked.connect(_do_save)
            cancel_btn.clicked.connect(dlg.reject)
            dlg.exec_()
        except Exception as e:
            QMessageBox.critical(self, "错误", f"打开价格配置失败: {e}")

    # ===== 商业化：激活码生成（发卡，写入 activation_codes 表） =====
    def open_activation_code_generator(self):
        try:
            from PyQt5.QtWidgets import QTextEdit
            from entitlement import generate_activation_codes
            dlg = QDialog(self)
            dlg.setWindowTitle("生成激活码")
            dlg.setMinimumWidth(440)
            dl = QVBoxLayout(dlg)
            dl.setContentsMargins(18, 18, 18, 18)
            dl.setSpacing(12)

            tip = QLabel("批量生成卡密激活码：用户在发卡平台付款后，在 PC-action「设置 → 会员中心」输入即用。")
            tip.setWordWrap(True)
            dl.addWidget(tip)

            form = QFormLayout()
            count_edit = QLineEdit("10")
            plan_combo = QComboBox()
            plan_combo.addItem("基础会员 (plan_1)", "plan_1")
            plan_combo.addItem("高级会员 (plan_2)", "plan_2")
            months_edit = QLineEdit("1")
            form.addRow("生成数量", count_edit)
            form.addRow("绑定档位", plan_combo)
            form.addRow("开通时长(月)", months_edit)
            dl.addLayout(form)

            result_edit = QTextEdit()
            result_edit.setReadOnly(True)
            result_edit.setMinimumHeight(150)
            result_edit.setPlaceholderText("生成的激活码将显示在此，可全选复制")
            dl.addWidget(result_edit)
            ...
            def _do_gen():
                nonlocal codes
                try:
                    n = int(count_edit.text() or 0)
                    months = int(months_edit.text() or 1)
                    plan = plan_combo.currentData()
                    if n <= 0 or n > 500:
                        QMessageBox.warning(dlg, "提示", "数量需在 1-500 之间")
                        return
                    codes = generate_activation_codes(n, months, plan, "")
                    if codes:
                        result_edit.setPlainText("\n".join(codes))
                        QMessageBox.information(dlg, "已生成", f"成功生成 {len(codes)} 个激活码")
                    else:
                        QMessageBox.warning(dlg, "失败", "生成失败，请确认 Supabase 已连接且 activation_codes 表已建")
                except Exception as e:
                    QMessageBox.warning(dlg, "失败", f"生成异常: {e}")

            def _do_copy():
                if codes:
                    QApplication.clipboard().setText("\n".join(codes))
                    QMessageBox.information(dlg, "已复制", f"已复制 {len(codes)} 个激活码")

            gen_btn.clicked.connect(_do_gen)
            copy_btn.clicked.connect(_do_copy)
            close_btn.clicked.connect(dlg.reject)
            dlg.exec_()
        except Exception as e:
            QMessageBox.critical(self, "错误", f"打开激活码生成失败: {e}")
    ...
    def init_ui(self):
        ...
        # ===== 商业化管理入口（价格配置 + 激活码生成） =====
        commer_layout = QHBoxLayout()
        commer_layout.setSpacing(10)
        commer_layout.setContentsMargins(12, 12, 12, 12)
        self.pricing_btn = QPushButton("价格配置")
        self.pricing_btn.setFixedHeight(32)
        self.pricing_btn.setStyleSheet("background-color:#5A6069;color:white;border:none;border-radius:8px;font-weight:bold;padding:0 16px;")
        self.pricing_btn.clicked.connect(self.open_pricing_editor)
        commer_layout.addWidget(self.pricing_btn)
        self.gen_code_btn = QPushButton("生成激活码")
        self.gen_code_btn.setFixedHeight(32)
        self.gen_code_btn.setStyleSheet("background-color:#5A6069;color:white;border:none;border-radius:8px;font-weight:bold;padding:0 16px;")
        self.gen_code_btn.clicked.connect(self.open_activation_code_generator)
        commer_layout.addWidget(self.gen_code_btn)
        commer_layout.addStretch()
        user_layout.addLayout(commer_layout)
        ...
        self.prev_page_btn.setFixedSize(70, 28)
        ...
        self.next_page_btn.setFixedSize(70, 28)
        ...
        self.batch_admin_btn.setFixedSize(min_button_width + 20, 32)
        ...
        self.batch_replay_btn.setFixedSize(min_button_width + 20, 32)
        ...
        self.batch_enable_btn.setFixedSize(min_button_width, 32)
        ...
        self.refresh_btn.setFixedSize(min_button_width + 10, 32)
        ...
        self.create_user_btn.setFixedSize(min_button_width, 32)
        ...
        self.refresh_feedback_btn.setFixedSize(min_button_width, 32)
        ...
        self.mark_processed_btn.setFixedSize(min_button_width + 20, 32)
        ...
        self.delete_feedback_btn.setFixedSize(min_button_width, 32)
...
class CreateUserDialog(QDialog):
    def __init__(self, parent=None):
        ...
        create_btn.setFixedSize(100, 32)
