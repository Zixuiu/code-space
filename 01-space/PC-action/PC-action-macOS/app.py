...
# 普通动作"等待出现"模式的图片等待超时（秒）：步间最多等待这么久让下一张目标图片出现，
# 超时即判执行失败。替代原来的"固定间隔(傻等)"。组合技不受影响，仍走固定间隔。
IMAGE_WAIT_TIMEOUT = 2.0
...
from PyQt5.QtCore import Qt, QTimer, QPoint, QPointF, QRectF, QEvent, QObject, QSize, QPropertyAnimation, QRect, QAbstractAnimation, QThread, QEasingCurve, QMimeData, pyqtSignal, QByteArray
...
def load_svg_icon(name, size=27, color=None):
    """加载 icons/ 目录下 SVG 图标，渲染为指定尺寸的 QIcon。
...
class FolderManager(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # 添加回收站按钮
        self.trash_button = QPushButton("回收站")
        self.trash_button.setIcon(load_svg_icon("trash", 18))
        self.trash_button.setIconSize(QSize(32, 32))
        ...
        for i, (name, ctime, path) in enumerate(folders):
            ...
            rename_btn.setStyleSheet(f"""
                QPushButton {{
                    background-color: transparent;
                    color: #5A6069;
                    boborder-radius: 8px                    font-weight: 600;
            ...
            # （流程右键菜单）弹不出来。给容器和按钮都挂上 CustomContextMenu，
    ...
    def view_images(self, folder_path):
        ...
        # 这两个控件被 monkey-patch 了 Python 事件方法，必须把 wrapper 挂到 dialog 上
        # 保持引用，否则函数返回后 Python 侧被 GC，点击/拖动会失效甚至 access violation
        dialog._red_dot = _red_dot
        dialog._dot_bar = _dot_bar
        ...
        if image_files or has_recording_json:
            ...
            add_btn.setIcon(load_svg_icon("plus", 16))
            add_btn.setIconSize(QSize(24, 24))
            add_btn.setFixedSize(140, 34)
            ...
            # 坐标录制按钮 - 用主题灰主按钮，和整体沉稳灰配色统一
            coord_btn = QPushButton("添加坐标操作")
            coord_btn.setIcon(load_svg_icon("mouse", 16))
            coord_btn.setIconSize(QSize(24, 24))
            coord_btn.setFixedSize(140, 34)
    ...
    def _populate_unified_rows(self, dialog, folder_path, list_layout):
        ...
        def _build_rows():
            ...
            for i, record in enumerate(recording_data):
                ...
                ds.setStyleSheet("QDoubleSpinBox { background: #FFFFFF; border: 1px solid rgba(0,0,0,0.06); border-radius: 4px; font-size: 11px; color: black; padding: 0; } QDoubleSpinBox:focus { border-color: #5A6069; }")
                dl.addWidget(ds)
                du = QLabel("秒")
                ...
                def _bd(w):
                    ...
                    def _mme(s, e):
                        ...
                        # PyQt5 要求传 Qt.DropAction 枚举，不能直接传 int（传 2 会抛 TypeError）
        ...
        def _delete_step(idx, img_file=None):
            ...
            if idx < 0 or idx >= len(recording_data):
                return
            reply = self.parent.show_beautiful_message(
                'question', "确认删除",
                f"确定要删除步骤 {recording_data[idx].get('step', idx+1)} 吗？",
    ...
    def _build_coord_table_panel(self, parent_window, folder_path, recording_json_path):
        ...
        table.setStyleSheet(
            "QTableWidget{background:#FFFFFF;border:1px solid #E5E5EA;border-radius: 4px;gridline-color:transparent;outline:none;}"
        ...
        def _make_param_widget(row_idx, record):
            ...
            if _at == 'text_input':
                ...
                _k = record.get('key', '')
                _lb = QLabel(f"按键 {_k}" if _k else "(空)")
                _lb.setStyleSheet("QLabel{color:#5A6069;font-size:11px;padding:2px 6px;background:rgba(90,96,105,0.1);border-radius:6px;}")
                _lb.setCursor(Qt.PointingHandCursor)
        ...
        add_btn.setIcon(load_svg_icon("plus", 16))
        add_btn.setIconSize(QSize(29, 29))
        add_btn.setStyleSheet("""
            QPushButton {
        ...
        add_menu.addAction("按键输入").triggered.connect(lambda: _add_key_op())
    ...
    def show_scroll_input_dialog(self, index, folder_path):
        ...
        try:
            ...
            self.parent.show_beautiful_message('critical', '错误', f'修改滚动设置失败: {e}')
    ...
    def browse_condition_image(self, line_edit, folder_path):
        ...
        select_btn.setFixedSize(100, 32)
    ...
    def open_trash(self):
        ...
        clear_btn.setStyleSheet("""
            ...
            QPushButton#clearTrashBtn:hover { background-color: #6B7178; }
            QPushButton#clearTrashBtn:pressed { background-color: #3E434B; }
        """)
    ...
    def permanent_delete_from_trash(self, row, table):
        ...
        try:
            ...
            self.show_beautiful_message('information', "成功", f"'{original_name}' 已永久删除", parent=self)
        except Exception as e:
    ...
    def clear_trash(self, table):
        ...
        try:
            ...
            self.show_beautiful_message('information', "成功", "回收站已清空", parent=self)
        except Exception as e:
...
class RoundedPillButton(QPushButton):
    ...
    def __init__(self, text="", bg_color="#5A6069", text_color="white", parent=None, font_size=None):
...
class TrashButtonDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        f = QFont("Microsoft YaHei")
        f.setPixelSize(13)          # 像素字号，不受系统 DPI 缩放影响
        f.setWeight(QFont.Medium)
        painter.setFont(f)
...
class AutoRecorderApp(QMainWindow):
    ...
    def create_image_grid(self, grid_layout, folder_path, parent_widget, dialog=None, step_action_map=None, max_cols=None):
        ...
        for step_num, img_path in step_images:
            ...
            btn_colors = {'left_click': '#8E8E93', 'right_click': '#8E8E93',
                         'double_click': '#8E8E93', 'keyboard': '#5A6069', 'drag': '#8E8E93'}
            pill_color = btn_colors.get(step_actual, '#8E8E93')
            btn.setStyleSheet(f"""
                QPushButton {{
                    background-color: {pill_color};
                    color: white;
                    borborder-radius: 8px                   font-weight: 600;
                    font-size: 11px;
                    padding: 0 12px;
                    text-align: center;
                }}
    ...
    def show_action_type_menu(self, button):
        ...
        for label, action_type in action_items:
            ...
            if action_type == current:
                action.setIcon(load_svg_icon("check", 16))
    ...
    def change_action_type(self, button, new_action_type):
        ...
        if new_action_type == 'right_click':
            reply = self.show_beautiful_message('question', '右击风险提示', '右击会弹出系统菜单，可能导致程序暂时无响应！\n'
                '建议：\n1. 优先Click\n2. 若必须右击，确保目标在前台\n'
    ...
    def _update_replay_ui(self):
        ...
        if hasattr(self, 'floating_replay_btn'):
            ...
            if self.replay_enabled:
                ...
                self.floating_replay_btn.setIcon(load_svg_icon("stop", 18))
    ...
    def create_record_tab(self):
        ...
        class PillButton(QPushButton):
            """iOS 风格药丸按钮 - 自绘确保绝对圆润"""
            def __init__(self, text="", color_top="#5A6069", color_mid="#5A6069", color_bottom="#3E434B",
        ...
        self.record_mode_combo.setStyleSheet("""
            ...
            QComboBox QAbstractItemView {
                ...
                border: 1px solid #D1D1D6;
                border-radius: 4px;
                selection-background-color: #5A6069;
        ...
        float_btn = PillButton("悬浮窗口",
                               color_top="#5A6069", color_mid="#5A6069", color_bottom="#3E434B",
                               text_color="white")
    ...
    def create_settings_tab(self):
        ...
        font_btn.setIcon(load_svg_icon("text", 18))
        font_btn.setIconSize(QSize(32, 32))
        ...
        # 快捷键设置
        shortcut_btn = QPushButton("快捷键设置")
        shortcut_btn.setIcon(load_svg_icon("keyboard", 18))
        shortcut_btn.setIconSize(QSize(32, 32))
        shortcut_btn.setStyleSheet(f"""
            QPushButton {{
                background: {THEME_PRIMARY};
                color: white;
        ...
        log_btn.setIcon(load_svg_icon("clipboard", 18))
        log_btn.setIconSize(QSize(32, 32))
        ...
        steps = [
            ...
            },
            {
                ...
                "content": """
                    ...
                    <p>&nbsp;&nbsp;&nbsp;&nbsp;• <span style="color: #5A6069;">⌨️ 按键</span>：修改按键</p>
                ...
                if i == step_idx:
                    ...
                    indicator.setStyleSheet("""
                        QPushButton {
                            background-color: #34C759;
                            color: white;
                            border-radius: 1border-radius: 8px              font-size: 16px;
                            font-weight: bold;
                        }
                    """)
                else:
                    indicator.setStyleSheet("""
                        QPushButton {
                            background-color: #D1D1D6;
                            color: #8E8E93;
                            border-radius: 18border-radius: 8px             font-size: 16px;
                            font-weight: bold;
                        }
                    """)
    ...
    def rename_folder_in_tab(self, folder_path, table_widget):
        ...
        new_name, ok = show_styled_input(self, "重命名", "请输入新名称：", text=old_name)
    ...
    def open_trash_dialog(self):
        ...
        # 与「查看图片」窗口底部一致：按钮居中（前后都加 stretch）
        btn_row.addStretch()

        # 与「查看图片」窗口底部按钮保持同一套扁平风：140x34 / 8px 圆角 / 13px 600 字重
        # 配色对齐参考样：恢复选中=深灰主色 / 永久删除=红 destructive 警告 / 清空回收站=白底灰边次要
        def _trash_btn_style(bg, fg, hover, pressed, border="none"):
            return f"""
                QPushButton {{
                    background-color: {bg};
                    color: {fg};
