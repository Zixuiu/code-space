...
class LoginDialog(QDialog):
    ...
    def handle_register(self):
        ...
        if strength_text == "弱":
            dlg = StyledMessageDialog(self, title="密码强度提示", text="您的密码强度较弱，建议使用包含大小写字母、数字和特殊字符的密码。\n是否继续注册？", msg_type="question", buttons="yes_no")
            dlg.exec_()
...
"""
    ...
    def _create_button_section(self, screen_width, screen_height, button_height, font_size):
        ...
        back_button = self.create_button(
            "返回登录",
            f"background-color: #5A6069; color: white; font-size: {font_size}px;",
            min_width=int(screen_width * 0.12),
            min_height=button_height,
