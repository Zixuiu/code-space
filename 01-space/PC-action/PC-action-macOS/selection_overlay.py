...
class SelectionOverlay(QWidget):
    ...
    def add_operation_to_list(self, operations_list, operation_type):
        ...
        if operation_type == "click":
            ...
            # 获取文本输入
            from beautiful_dialog import show_styled_input
            text, ok = show_styled_input(self, "输入文本", "请输入要输入的文本：")
            if ok and text:
                operation = {
                    "action_type": "text_input",
                    "text": text,
            ...
            # 获取延迟时间
            from beautiful_dialog import show_styled_double
            delay, ok = show_styled_double(self, "设置延迟", "请输入延迟时间（秒）：",
                                           value=0.5, min_value=0, max_value=60, decimals=1, step=0.1)
            if ok:
                operation = {
                    "action_type": "delay",
                    "delay": delay
