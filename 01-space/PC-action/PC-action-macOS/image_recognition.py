    ...
    for i, operation in enumerate(recording_data):
        ...
        try:
            ...
            if action_type in ['text_input', 'keyboard', 'keyboard_direct', 'scroll']:
                ...
                # 步间等待策略（仅"下一步是图片识别"才用"等待出现"，其余正常盲等）：
                #  - 下一步是图片：普通动作(wait_for_image)→轮询该图 IMAGE_WAIT_TIMEOUT 内出现，超时判失败；组合技→固定间隔盲等。
                #  - 下一步非图片（键盘/文本/滚动）：两种模式都正常盲等（delay→replay_interval），不套"等待出现"。
                if i < total_operations - 1:  # 不是最后一个操作
                    ...
                    if _next_image and wait_for_image:
                        # 仅图片识别步骤之间：把固定等待替换为"等待出现"
            ...
            if image_name:
                ...
                if not location:
                    ...
                    # 提速策略：形状分极高(>=_STABLE_SKIP_SCORE)通常是静止清晰图标，移动中的动画/模糊帧极少达此分，
                    # 直接点击（零额外耗时）；只有中低分（疑似动画帧/边缘）才做轻量稳定确认，避免拖累整体速度。
                    if not turbo_match:
                        try:
                            ...
                            if _hit >= _STABLE_SKIP_SCORE:
                                ...
                                for _s in range(3):
                                    time.sleep(0.03)
                                    _rel = find_image_with_timeout(image_path, confidence=dynamic_confidence, timeout=0.06,
            ...
            # ★★★ 关键修复：点击操作（left_click/right_click/double_click等）完成后，如果剪贴板发生了变化，
            #               说明是用户主动点了某个"复制按钮"或"WPS单元格复制"等非Ctrl+C触发的复制，
            ...
            # 步间等待（图片动作成功路径，仅"下一步是图片识别"才用"等待出现"）：
            #  - 下一步是图片：普通动作(wait_for_image)→轮询该图 IMAGE_WAIT_TIMEOUT 内出现，超时判失败；组合技→固定间隔盲等。
            #  - 下一步非图片（键盘/文本/滚动）：两种模式都正常盲等，不套"等待出现"。
...
def find_image_with_timeout(image_path, confidence=0.8, timeout=0.5, consider_color=True, region_center=None, stop_check=None, roi_hint=None, strict=False, skip_small_match=False):
    ...
    _LAST_MATCH_BEST_SCORE_GLOBAL[1] = None
    ...
    # [已取消位置锚定] 之前按录制坐标做 220px 硬约束，图标实际位置变动时会误拒导致点不到，现已移除。
        ...
        if _best_score < _early_threshold:
            ...
            try:
                ...
                for _k, _v in sorted(scale_best_scores.items(), key=lambda x: x[1][0], reverse=True):
                    _bl_save = _v[1]
                    break
                _LAST_MATCH_BEST_SCORE_GLOBAL[0] = float(_best_score) if _best_score else 0.0
                _LAST_MATCH_BEST_SCORE_GLOBAL[1] = _bl_save
            except Exception:
                pass
            return None
        ...
        if (stop_check and stop_check()) or (stop_check is None and _replay_stop_flag):
            ...
            try:
                if scale_best_scores:
                    _bs_stop = max(v[0] for v in scale_best_scores.values())
                    _bl_stop = None
                    for _k, _v in sorted(scale_best_scores.items(), key=lambda x: x[1][0], reverse=True):
                        _bl_stop = _v[1]
                        break
                    _LAST_MATCH_BEST_SCORE_GLOBAL[0] = float(_bs_stop) if _bs_stop else 0.0
                    _LAST_MATCH_BEST_SCORE_GLOBAL[1] = _bl_stop
            except Exception:
                pass
            return None
        _loop_iter += 1
    ...
    if scale_best_scores:
        try:
            _bs = max(v[0] for v in scale_best_scores.values())
            _bl = None
            for _k, _v in sorted(scale_best_scores.items(), key=lambda x: x[1][0], reverse=True):
                _bl = _v[1]
                break
            _LAST_MATCH_BEST_SCORE_GLOBAL[0] = float(_bs) if _bs else 0.0
            _LAST_MATCH_BEST_SCORE_GLOBAL[1] = _bl
        except Exception:
            pass
    return None


_LAST_MATCH_BEST_SCORE_GLOBAL = [0.0, None]
def get_last_match_best_score():
    return float(_LAST_MATCH_BEST_SCORE_GLOBAL[0])
