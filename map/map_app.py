    ...
    <script>
        ...
        async function addAddress() {
            ...
            for (let i = 0; i < parsedAddresses.length; i++) {
                ...
                document.getElementById('mapInfo').textContent = `正在解析: ${i + 1}/${total} (已成功 ${successCount}, 失败 ${failCount})`;
                try {
                    let shopName = '';
                    ...
                    failedAddresses.push(address);
                    failCount++;
                    console.error('地址请求错误:', address, error);
                }
            }
    ...
    if route and len(route) >= 2:
        ...
        for i in range(len(route) - 1):
            ...
            if idx1 < len(addresses) and idx2 < len(addresses):
                ...
                p2 = addresses[idx2]
                # 绘制细线（带圆角端点）
                svg_parts.append(f'<line x1="{p1["_cx"]}" y1="{p1["_cy"]}" x2="{p2["_cx"]}" y2="{p2["_cy"]}" stroke="#FF6B6B" stroke-width="1.5" stroke-linecap="round" opacity="0.7"/>')
                # 绘制箭头
...
def draw_arrow(svg_parts, x1, y1, x2, y2):
    ...
    # 箭头终点（在线段75%位置）
    arrow_x = round(x1 + (x2 - x1) * 0.75, 2)
    arrow_y = round(y1 + (y2 - y1) * 0.75, 2)
    ...
    svg_parts.append(f'<polygon points="{arrow_x},{arrow_y} {x3},{y3} {x4},{y4}" fill="#FF6B6B" opacity="0.8"/>')
...
'''
    ...
    for i in route:
        addr = addresses[i]
        shop_name = addr.get('shop_name', '')
        addr_name = addr.get('name', '未知')
        if shop_name:
            copy_lines.append(f"{step}. 【{shop_name}】{addr_name}")
        else:
            copy_lines.append(f"{step}. {addr_name}")
        step += 1
    ...
    for idx, line in enumerate(copy_lines):
        ...
        if idx < len(copy_lines) - 1:
            final_lines.append("⬇️")
