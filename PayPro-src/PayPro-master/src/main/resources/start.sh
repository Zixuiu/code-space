...
then
    ...
    EXTRA_JAVA_OPTIONS="org.springframework.boot.loader.JarLauncher --server.port=${server_port} --spring.config.additional-location=file:${APP_DIR}/config/  "
else
    echo "profiles is not empty"
    EXTRA_JAVA_OPTIONS="org.springframework.boot.loader.JarLauncher --server.port=${server_port}  --spring.profiles.active=${profiles} --spring.config.additional-location=file:${APP_DIR}/config/  "
fi
